...
\title{\textbf{Assignment Submission for Intern — ML \& Speech Data Pipeline}\\
\large YouTube Speech Dataset Pipeline Report}
\author{Rushabh Lodha\\
\href{mailto:rishilodha03@gmail.com}{rishilodha03@gmail.com}}
\date{June 2026}
...
\noindent Public deliverables:
\begin{itemize}
    \item HuggingFace dataset: \href{https://huggingface.co/datasets/Rushabh3/sarvam-indian-tts-60min}{\texttt{Rushabh3/sarvam-indian-tts-60min}}.
    \item GitHub repository: \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline}{\texttt{Rushabh333/sarvam-indian-tts-pipeline}}.
    \item Final metadata is published through the HuggingFace dataset splits and documented in the GitHub repository.
\end{itemize}

\noindent The main public analysis artifacts in the repository are:
\begin{itemize}
    \item \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/analysis/before_after_manual_review.csv}{\texttt{analysis/before\_after\_manual\_review.csv}}: manual review sheet for raw/final clip pairs.
    \item \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/analysis/before_after_metrics/before_after_report.md}{\texttt{analysis/before\_after\_metrics/before\_after\_report.md}}: audio before/after metrics.
    \item \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/analysis/before_after_metrics/transcript_report.md}{\texttt{analysis/before\_after\_metrics/transcript\_report.md}}: transcript coverage and preservation metrics.
    \item \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/analysis/emotion_tag_repair/emotion_tag_repair_report.md}{\texttt{analysis/emotion\_tag\_repair/emotion\_tag\_repair\_report.md}}: emotion tag repair report.
    \item \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/report/final_report.pdf}{\texttt{report/final\_report.pdf}}: compiled PDF report.
\end{itemize}
...
The pipeline is organized as a sequence of scripts under \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/tree/main/pipeline}{\texttt{pipeline/}}:
...
The English preservation results are strong. The Hindi/code-mixed results are more complicated: many low-scoring rows show script instability in ASR output rather than obvious audio corruption. For this reason, I did not use the ASR similarity score as a hard rejection metric for Hindi. Instead, low-similarity rows are written to \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/analysis/before_after_metrics/low_asr_similarity_review.csv}{\texttt{low\_asr\_similarity\_review.csv}} for manual review.
...
\section{Manual Review Form}

Because automated metrics do not fully capture TTS suitability, I also created a Google Form for human review of a 37-clip subset. The review set was selected to be language-balanced and source-diverse while still prioritizing rows that needed human attention: low ASR-similarity clips, lower speaker-similarity clips, and rows marked for emotion review. The form uses public HuggingFace audio links for both the final clip and its matched raw-before clip.

\noindent Public review form:
\begin{center}
\href{https://docs.google.com/forms/d/e/1FAIpQLSdWRh_ek1CADZmEORGfKX47R4YiNFZ44hmqzWTBAdvha5qeEA/viewform}{\texttt{Sarvam TTS Manual Review - 37 Clips}}
\end{center}

\begin{table}[H]
\centering
\caption{Manual review subset}
\begin{tabular}{lr}
\toprule
\textbf{Item} & \textbf{Value} \\
\midrule
Review clips & 37 \\
Hindi/code-mixed review clips & 21 \\
Indian English review clips & 16 \\
Audio per clip & Final clip + matched raw-before clip \\
Review pages & 37 clip pages + introduction page \\
Public audio host & HuggingFace dataset repository \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Manual review rubric}
\begin{tabular}{p{0.29\linewidth}p{0.58\linewidth}}
\toprule
\textbf{Question} & \textbf{Purpose} \\
\midrule
Audio clarity, 1--5 & Measures noise, music bleed, room tone, and general listenability. \\
Speaker consistency & Checks whether the clip is single-speaker or has overlap/other speakers. \\
Boundary quality & Captures cut starts, cut endings, incomplete sentences, clicks, and pops. \\
Speech naturalness & Flags read-aloud, audiobook-style, synthetic, or heavily processed speech. \\
Dominant emotion/register & Human check for neutral, excited, somber, frustrated, formal, casual, or expressive register. \\
Final decision & Converts ratings into an actionable label: keep, keep after trim, reject, or second review. \\
\bottomrule
\end{tabular}
\end{table}

At report time, the form had been created and verified, but a complete human response set had not yet been collected. I therefore do not report fabricated human acceptance rates or reviewer scores. The form is included as a reproducible review instrument, and the response sheet can be used later to compute keep rate, boundary-fix rate, mean clarity, and disagreement between heuristic emotion tags and human labels.
...
\appendix
\section{Public Artifact Links}

\begin{itemize}
    \item HuggingFace dataset: \href{https://huggingface.co/datasets/Rushabh3/sarvam-indian-tts-60min}{\texttt{Rushabh3/sarvam-indian-tts-60min}}
    \item GitHub repository: \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline}{\texttt{Rushabh333/sarvam-indian-tts-pipeline}}
    \item Before/after metrics CSV: \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/analysis/before_after_metrics/before_after_metrics.csv}{\texttt{analysis/before\_after\_metrics/before\_after\_metrics.csv}}
    \item Transcript similarity CSV: \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/analysis/before_after_metrics/asr_text_similarity.csv}{\texttt{analysis/before\_after\_metrics/asr\_text\_similarity.csv}}
    \item Low transcript similarity review CSV: \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/analysis/before_after_metrics/low_asr_similarity_review.csv}{\texttt{analysis/before\_after\_metrics/low\_asr\_similarity\_review.csv}}
    \item Emotion review CSV: \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/analysis/emotion_tag_repair/emotion_manual_review_v2.csv}{\texttt{analysis/emotion\_tag\_repair/emotion\_manual\_review\_v2.csv}}
    \item Manual review Google Form: \href{https://docs.google.com/forms/d/e/1FAIpQLSdWRh_ek1CADZmEORGfKX47R4YiNFZ44hmqzWTBAdvha5qeEA/viewform}{\texttt{Sarvam TTS Manual Review - 37 Clips}}
    \item Manual review manifest: \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/review_form/selected_37_review_manifest.csv}{\texttt{review\_form/selected\_37\_review\_manifest.csv}}
    \item Source list: \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/sources/sources.jsonl}{\texttt{sources/sources.jsonl}}
    \item Iteration 1 audit: \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/docs/iteration_1_audit_and_pipeline_hardening.md}{\texttt{docs/iteration\_1\_audit\_and\_pipeline\_hardening.md}}
    \item Final PDF report: \href{https://github.com/Rushabh333/sarvam-indian-tts-pipeline/blob/main/report/final_report.pdf}{\texttt{report/final\_report.pdf}}
\end{itemize}
